        ...
        public function proxySnapshot(Request $request)
{
    try {
        ...
        // Use Guzzle with connection pooling for better performance
        $response = Http::withOptions([
            'verify' => false, // Skip SSL verification for faster responses
            'connect_timeout' => 3,
            'timeout' => 5,
            'stream' => false
        ])->withHeaders([
        ...
        Log::error("❌ Proxy failed with status: " . $response->status());
        return response()->json(['error' => 'Failed to fetch snapshot'], $response->status());
